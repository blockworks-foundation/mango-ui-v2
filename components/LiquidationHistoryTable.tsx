...
import useLiquidationsHistory from '../hooks/useLiquidationsHistory'
import { LinkButton } from './Button'
import Tooltip from './Tooltip'

const LiquidationHistoryTable = () => {
  const [showLiquidationDetail, setShowLiquidationDetail] = useState(null)
  const { liquidationsHistory } = useLiquidationsHistory()
  const { items, requestSort, sortConfig } =
    useSortableData(liquidationsHistory)
      ...
      <div className={`-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8`}>
        <div className={`align-middle inline-block min-w-full sm:px-6 lg:px-8`}>
          {liquidationsHistory ? (
            ...
            <>
              <div className="animate-pulse bg-th-bkg-3 h-14 mb-2 w-full" />
              <div className="animate-pulse bg-th-bkg-3 h-14 mb-2 w-full" />
              <div className="animate-pulse bg-th-bkg-3 h-14 w-full" />
            </>
          )}
        </div>
      </div>
